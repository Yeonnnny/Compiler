...
char *opcode_name[]={"OP_NULL", "LOD","LDX","LDXB", "LDA","LITI",
	...
	"NOT", "OR", "AND", "CVTI","CVTF", "JPC","JPCR", "JMP","JPT","JPTR",
	"INT","INCI","INCF","DECI","DECF","SUP","CAL","ADDR","RET",
	   ...
	   case N_EXP_IDENT :
		...
		switch (id->kind) {
		   ...
		   case ID_FUNC:
			...
			break;
		   default:
			gen_error(13, node->line, node->name);
			break;
...
/*
	if (isIntType(t)) {
        	// 정수형 변수 초기화 코드 생성
        	gen_expression(node); // 초기화 값을 스택에 푸시
        	gen_code_i(STX, 0, addr); // 스택에서 값을 꺼내 변수에 저장
    	}
*/
}
